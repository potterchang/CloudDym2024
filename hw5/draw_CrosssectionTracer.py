...
def drawCross(casename, itime):
    ...
    trs = [tr01, NO2, O3]
    names = ["Tracer","NO2","O3"]
    levels_list = [np.arange(2,20.1,2), np.arange(10,100.1,5), np.arange(5,60.1,5)]
    ...
    ax[0].set_title(f"{casename}\n y=12.8km Cross-section", loc="right",fontsize=10)
    ...
    for i in range(3):
        ...
        xgap = 1; zgap=3
        wsmask = (u**2+w**2)**0.5 <= 1.0
        um = u.copy(); wm = w.copy()
        um[wsmask] = np.nan
        wm[wsmask] = np.nan
        QV=ax[i].quiver(xc[::xgap], zc[::zgap], um[::zgap,::xgap], wm[::zgap,::xgap], scale=100)
        if i ==0:
            QVK=ax[i].quiverkey(QV, 1.05, 1.1, 2, '2 m/s', coordinates='axes', 
    ...
    ax[i].set_xlim([8000-800, 17600+800])
    ...
    foldername = f"crossTracer/{casename}/Y64"
...
drawCross("pbl_half_PU_1", itime=220)
