...
def drawCross(casename, itime):
    ...
    trs = [tr01, NO2, O3]
    names = ["Tracer","NO2","O3"]
    levels_list = [np.arange(1.0,5.1,0.5), np.arange(2,24.1,2), np.arange(15,60.1,5)]
    ...
    ax[0].set_title(f"{casename}\n y=12.8±3.2km Avg. Cross-section", loc="right",fontsize=10)
    ...
    ax[i].set_xlim([8000-1600, 17600+1600])
    ax[-1].set_ylim([0,1300])
    ax[-1].set_xlabel('xc [m]')
    ax[-1].set_xticks(np.arange(8000-1600,17600.1+1600, 1600))
...
drawCross("pbl_half_PU_1", itime=360)
