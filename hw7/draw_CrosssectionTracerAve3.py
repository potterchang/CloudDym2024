...
def loadData(casename, itime):
    ...
    tr01 = VVMLoader.loadVariable("tr02", itime, casename, zIdx=np.arange(50), yIdx=ind_y, xIdx=np.arange(128), TOPOmask=False, regrid=True)
...
def drawCross(casename, itime):
    ...
    trs = [tr01, NO2, O3]
    names = ["Tracer","NO2","O3"]
    levels_list = [np.arange(1.0,6.51,0.5)*5, np.arange(2,20.1,2)*100, np.arange(5,50.1,5)]
    # tr01: np.arange(1.0,5.51,0.5)*200,
    # tr02: np.arange(1.0,5.51,0.5)*50,
    ...
    fig, ax = plt.subplots(1,1, figsize=[6,4] , sharex=True,sharey=True, dpi=300)
    plt.subplots_adjust(hspace=0.25)
    ax.set_title(f"{casename[9:]}\n y-axis avg. Cross-section", loc="right",fontsize=10)
    ax.set_title(f"{HHMM}", fontsize=16)

    ax.set_title(f"{names[0]} [{unit_list[0]}]", fontsize=14, loc='left', weight='bold')
    ax.set_ylabel('zc [m]')
    
    levels = levels_list[0]
    cmap = plt.cm.get_cmap(cmap_list[0], len(levels)+1)
    norm = BoundaryNorm(levels, ncolors=len(levels)+1, clip=False, extend='max')
    PC = ax.pcolormesh(xc, zc, trs[0], cmap=cmap, norm=norm, shading='nearest')
    ...
    um[wsmask] = np.nan
    wm[wsmask] = np.nan
    QV=ax.quiver(xc[::xgap], zc[::zgap], um[::zgap,::xgap], wm[::zgap,::xgap], scale=40, width=0.005)

    QVK=ax.quiverkey(QV, 1.06, 1.04, 1, '1 m/s', coordinates='axes', 
                            labelsep=0.08, fontproperties={'size':9.5})

    ax.plot(xc, BLH1, 'grey', lw=2.5)
    ax.axvline(x=12800,color='r')
    ...
    ax.set_xlim([0, 25600])
    ax.xaxis.set_major_locator(MultipleLocator(6400))
    ax.set_ylim([0,1800])
    ax.set_xlabel('xc [m]')

    BLH2legend = Line2D([0], [0], linestyle='-', color='grey',linewidth=2.5),
    ax.legend([BLH2legend],
    ...
    foldername = f"crossTracer/{casename}/tr02"
    os.makedirs(foldername, exist_ok=True)
    imgname = f"crossTracer-tr02-{casename}-{str(itime).zfill(6)}.png"
    plt.savefig(f"{foldername}/{imgname}", bbox_inches="tight")
    plt.show()
    return imgname

#drawCross(casename, itime=220)
drawCross("pbl_half_PU_uarea_1", itime=500)

#%%
starttime = time.time()
if __name__ == '__main__':
    
    for casename in ["pbl_half_PU_uarea_1","pbl_half_PU_uarea_2"]:
        try:
